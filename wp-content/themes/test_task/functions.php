...
function test_task_setup() {
	...
	register_nav_menus(
		array(
			'menu-header' => esc_html__('Menu Header', 'test_task'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'test_task_custom_background_args',
			array(
...
function test_task_scripts() {
	...
	wp_enqueue_style( 'test_task-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'test_task-style', 'rtl', 'replace' );



	wp_deregister_script('jquery');
	wp_register_script('jquery', get_template_directory_uri() . '/js/jquery-3.6.0.min.js');
	wp_enqueue_script( 'network-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
	wp_enqueue_script( 'aos', get_template_directory_uri() . '/js/aos.js', array(), _S_VERSION, true );
	wp_enqueue_script( 'network-swiper', get_template_directory_uri() . '/js/swiper-bundle.min.js', array('jquery'), _S_VERSION, true );
	wp_enqueue_script( 'network-custom', get_template_directory_uri() . '/js/custom.js', array('jquery'), _S_VERSION, true );
